import TS1bis.FiniteOrderedPartitions
import TS1bis.Shellability
import TS1bis.WeakBruhatOrder
import TS1bis.EulerPoincareCharacteristic
...
s ∈ (CoxeterComplex α).facets ↔ IsLinearOrder α (powersetToPreorder ↑s).le := by
  ...
  . intro hsf
    ...
    simp only [Equiv.invFun_as_coe, OrderIso.toEquiv_symm, RelIso.coe_toEquiv, Equiv.toFun_as_coe,
  ...
  . intro hlins
    ...
    have hqp : q.1 ≤ p.1 := by
      change p ≤ q
      simp only [Equiv.toFun_as_coe, RelIso.coe_toEquiv, map_le_map_iff, Subtype.mk_le_mk,
        Finset.le_eq_subset]
      exact hst
    ...
    have heq' : q = p := by
      rw [←SetCoe.ext_iff]
      exact heq
    simp only [Equiv.toFun_as_coe, RelIso.coe_toEquiv, EmbeddingLike.apply_eq_iff_eq,
...
Finset.card s.1 = Fintype.card α -1 := by
  ...
  erw [and_iff_left (@Quotient.surjective_Quotient_mk'' _ (AntisymmRel.setoid α p.1.le))]
  intro a b hab
...
(WeakBruhatOrder_facets r).le s t  → so.le s t) : IsShellingOrder so :=
  ShellableofDecomposable (CoxeterComplex_is_decomposable r) (fun s t hst => hcomp (WeakBruhat_compatible_with_DF r s t hst)) sorry
