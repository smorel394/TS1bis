...
le_refl x := by by_cases hx : x = a
                . simp only [hx, dite_eq_ite, ite_true]
                . simp only [hx, dite_eq_ite, ite_false, ite_self]
...
/- A total essentially locally finite preorder is a SuccOrder (and also a PredOrder, but I didn't write it). This is because the
...
/-  set x:= WellFounded.min ((@Finite.Preorder.wellFounded_gt _ hfin (@Subtype.preorder _ (@PartialOrder.toPreorder _
     (@instPartialOrderAntisymmetrization α s)) _))) Set.univ (@Set.univ_nonempty _ hne)
  have hxE := x.2
