import TS1bis.general_preorder_stuff
...
(@instPartialOrderAntisymmetrization α s))
...
∃ (a : α), ∀ (b : α), ¬(s.lt a b) := by
  ...
  have hane : (@Set.univ (Antisymmetrization α s.le)).Nonempty := by
    existsi toAntisymmetrization s.le (Classical.choice hne)
    simp only [Set.mem_univ]
  existsi ofAntisymmetrization s.le sorry--(WellFounded.min (@Finite.Preorder.wellFounded_gt _ hfin _) _ hane)
  intro b
