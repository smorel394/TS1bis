import TS1bis.preorder_to_powerset
...
def dual (r : LinearOrder α) := @OrderDual.instLinearOrder α r
...
r.le a b → r.le a c → s.le b c := by sorry
...
def ReverseProductOrder (s : Preorder α) := @Prod.instPreorderProd _ _ (@OrderDual.instPreorder _ s) s
...
| ⟨a, b, habinv⟩ =>
    ...
    have hwf : WellFounded (fun (x y : A × A) => x < y) := sorry --@Finite.Preorder.wellFounded_lt (A × A) _ _
